...
public class ArduinoNoiseSensor extends ArduinoSensor{
	
	

	public ArduinoNoiseSensor(String name, String id) {
		super(name, id);
	}
	...
	public String getArduinoLoopCode() {
		...
		return "\n\t" + "// Loop code for " + name +"\n"
				...
				+ "\n\t"+ "if (digitalRead(" + id1 + "_pin)) {" + id1 + "_value = " + id1 + "_min_range;"
				+ "\n\t\t"+ "inputData(" + id1 + "_value, " + id1 + "_value_old, \"" + id1 + "\", first_loop);"
				+ "\n\t\t"+ "" + id1 + "_value_old = " + id1 + "_value;"
				+ "\n\t"+ "} else{"+ id1 + "_value = " + id1 + "_max_range;}"
				+ "\n\t\t"+ "inputData(" + id1 + "_value, " + id1 + "_value_old, \"" + id1 + "\", first_loop);"
				+ "\n\t\t"+ "" + id1 + "_value_old = " + id1 + "_value;";}		
